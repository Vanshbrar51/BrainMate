...
export async function GET(
  ...
  return withSpan("api.writeright.job.poll", async () => {
    ...
    if (!UUID_RE.test(jobId)) {
      return NextResponse.json(
        { error: "Invalid job ID", code: "INVALID_ID" },
        { status: 400 },
      );
    }
    ...
    if (job.status === "completed") {
      // If we have output in Supabase, try to return it
      // Also try Redis stream as it may have been written there
      let result = null;
      try {
        result = await readJobResult(jobId);
      } catch {
        // Redis unavailable — use Supabase output
      }
