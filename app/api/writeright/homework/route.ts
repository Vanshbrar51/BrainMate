...
export async function POST(req: Request) {
  return withErrorHandler(req, async () => {
    return withSpan("api.writeright.homework.post", async () => {
      ...
      addSpanAttributes({ "user.id": userId, "module": "homework" })

      const traceHeaders = injectTraceContext(new Headers())

      const pythonRes = await fetch(`${PYTHON_WORKER_URL}/study-mate`, {
        method: "POST",
        headers: {
