...
export async function POST(req: Request) {
  return withErrorHandler(req, async () => {
    return withSpan("api.writeright.interview.post", async () => {
      ...
      addSpanAttributes({ "user.id": userId, "module": "interview" })

      const traceHeaders = injectTraceContext(new Headers())

      const pythonRes = await fetch(`${PYTHON_WORKER_URL}/interview-pro`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({ prompt, userId, session_id, traceparent: traceHeaders.traceparent }),
      })
