...
export async function POST(req: Request) {
  return withErrorHandler(req, async () => {
    return withSpan("api.writeright.bug-explainer.post", async () => {
      ...
      addSpanAttributes({ "user.id": userId, "module": "bug-explainer" })

      const traceHeaders = injectTraceContext(new Headers())

      const pythonRes = await fetch(`${PYTHON_WORKER_URL}/dev-helper`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({ prompt, userId, traceparent: traceHeaders.traceparent }),
      })
