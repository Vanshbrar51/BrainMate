...
def mock_router():
    with patch("app.services.ai_worker.get_model_router") as mock_get:
        ...
        router.route_stream.return_value = make_response(
            '{"improved_text": "Safe", "teaching": {"mistakes": ["M"], "better_versions": ["V"], "explanations": ["E"]}, "follow_up": "...", "suggestions": ["A"], "scores": {"clarity": 10, "tone": 10, "impact": 10, "verdict": "Ready to send"}}'
        )
        yield router
...
def mock_external_services():
    with patch("app.services.ai_worker.get_chat_history", return_value=[]), patch(
        "app.services.ai_worker.save_ai_message", return_value=None
    ), patch("app.services.ai_worker.update_job_status", return_value=None), patch(
        "app.services.ai_worker.record_usage", return_value=None
    ), patch(
        "app.services.ai_worker.update_streak_and_achievements", return_value=None
    ), patch(
        "app.services.ai_worker.get_writing_profile", return_value=[]
    ), patch(
        "app.services.ai_worker.update_chat_title", return_value=None
    ), patch(
        "app.services.ai_worker.get_embedding_service"
    ) as mock_emb, patch(
        "app.services.ai_worker.match_voice_examples", return_value=[]
    ):
...
@pytest.mark.asyncio
async def test_worker_handles_detected_injection(
    base_job, mock_router, mock_external_services
):
    ...
    with patch("app.services.ai_worker.build_messages") as mock_build:
        ...
        mock_router.route_stream.return_value = make_response(
            '{"improved_text": "Neutralized", "teaching": {"mistakes": ["M"], "better_versions": ["V"], "explanations": ["E"]}, "follow_up": "...", "suggestions": ["A"], "scores": {"clarity": 10, "tone": 10, "impact": 10, "verdict": "Ready to send"}}'
        )
...
@pytest.mark.asyncio
async def test_worker_handles_truncated_json(
    base_job, mock_router, mock_external_services
):
    ...
    with patch(
        "app.services.ai_worker._repair_json_response", new_callable=AsyncMock
    ) as mock_repair:
...
@pytest.mark.asyncio
async def test_quality_gate_blocks_invalid_input(
    base_job, bad_input, reason_match, mock_external_services, mock_router
):
    ...
    all_teaching_text = " ".join(
        result.teaching.mistakes + result.teaching.explanations
    ).lower()
...
@pytest.mark.asyncio
async def test_critique_pipeline_failure_fallback(
    base_job, mock_router, mock_external_services
):
    with patch.object(app_settings, "enable_critique_pipeline", True):
        mock_router.route_stream.return_value = make_response(
            '{"improved_text": "Draft", "teaching": {"mistakes": ["M"], "better_versions": ["V"], "explanations": ["E"]}, "follow_up": "...", "suggestions": ["A"], "scores": {"clarity": 5, "tone": 5, "impact": 5, "verdict": "Needs more work"}}'
        )
