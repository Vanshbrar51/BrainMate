...
from app.models.job import VoiceListResponse
...
async def test_voice_ingest_unauthorized(client: AsyncClient):
    response = await client.post(
        "/voice/ingest", json={"content": "test", "user_id": "123"}
    )
...
async def test_voice_ingest_empty_content(client: AsyncClient):
    ...
    response = await client.post(
        "/voice/ingest", json={"content": "", "user_id": "123"}, headers=headers
    )
...
async def test_voice_ingest_success(client: AsyncClient):
    ...
    with patch("app.routers.voice.get_embedding_service") as mock_get_emb, patch(
        "app.routers.voice.save_voice_example", new_callable=AsyncMock
    ) as mock_save:
        ...
        response = await client.post(
            "/voice/ingest", json=request_data, headers=headers
        )

        assert response.status_code == 200
        assert response.json()["status"] == "success"
        mock_save.assert_called_once()
        mock_emb_service.get_embedding.assert_called_with(
            "This is a brand voice example."
        )
...
async def test_voice_list_success(client: AsyncClient):
    ...
    with patch(
        "app.routers.voice.get_voice_examples", new_callable=AsyncMock
    ) as mock_get:
        mock_get.return_value = mock_examples

        # Fixed URL: changed /voice to /voice/examples
        response = await client.get(
            f"/voice/examples?user_id={user_id}", headers=headers
        )
...
async def test_voice_delete_success(client: AsyncClient):
    ...
    with patch(
        "app.routers.voice.delete_voice_example", new_callable=AsyncMock
    ) as mock_delete:
        # Fixed URL: changed /voice/{id} to /voice/examples/{id}
        response = await client.delete(
            f"/voice/examples/{example_id}?user_id={user_id}", headers=headers
        )
